  ...
  async generateBulk(dto: GenerateTemplateSeatsDto) {
    try {
      ...
      if (!dto.seatMap?.length) {
        throw new BadRequestException('Seat map must contain at least one row');
      }
      ...
      this.logger.log(`Generating seats for template=${dto.templateId}`);
      ...
      this.logger.log(`Created ${result.count} template seats`);
